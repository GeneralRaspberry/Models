...
The proliferation of novel vector-borne diseases is directly linked to climate change. Here, I develop a spatially stochastic epidemic simulation model with vector-borne transmission between plants. I increase the abundance of vectors (v) and the attractiveness of infected plants (A), and measure the numbers of successful epidemics (infected plants >= 5). I implement 250 simulations per combination of v and A, with a time step of 14 days using Gillespie's τ-leap algorithm. The landscapes were generated using a Poisson-Mátern process, with a randomisation factor. My results show that an effective reproduction index can be approximated analytically from this model, indicating that the force of infection is partially a product of the distance between plants, vector abundance, vector preference and the rate at which plants are removed. This work provides a potential framework for future surveillance modelling, with critical information related to surveillance monitoring.
...
This paper seeks to shed light on one particular model of vector abundance, and how it largely determines the onset or extinquishment of an epidemic according to the well established idea of an effective reproduction ratio (Wadkin et al., 2024). This is typically calculated in simulation models by the number of individuals a single plant will infect during the infection period. However, computation can be expensive, and mechanistic determinations or approximations of this value are invaluable to epidemiological modellers, especially in the context of surveillance.

The model used in this paper assumes a multinomial distribution of insect vectors across a field of plants, which has the attractive qualities of being both easy to compute and naturally incorporates an attractiveness index for compartmental modelling of epidemics. Subsequently, the plant pathogen is transmitted via a gamma dispersal kernel which has the advantage of being flexible in construct and could potentially reflect numerous pathogen species. Reff is calculated using a simple SIR modelling approach. 

Methodology

This is a spatially-stochastic compartmental model, with transitions between Susceptible, Infected and Removed plants.
...
The time step in this model is determined by Gillespie's τ-leap algorithm, with transitions approximated using Poisson random variables,
